...
func TestInfoReportsPhase3R4Release(t *testing.T) {
	h := NewHandler(app.NewState())
	w := httptest.NewRecorder()
	...
	body := w.Body.String()
	for _, want := range []string{`"version":"0.4.3-phase3-r4"`, "standalone offline structure modeling", "no WSL or local solver required"} {
