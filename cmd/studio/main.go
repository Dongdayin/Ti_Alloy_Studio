...
func manualFile(w http.ResponseWriter) {
	...
	if err != nil {
		http.Error(w, "manual unavailable", 404)
		return
	}
...
func runEngineSmoke(path string) int {
	...
	if len(r.Engines) < 2 {
		payload["status"] = "FAIL"
		payload["error"] = "managed engine reports missing"
	}
	for _, er := range r.Engines {
		if er.Status != "PASS" {
			payload["status"] = "FAIL"
			payload["error"] = fmt.Sprintf("%s: %s", er.Name, er.Message)
			break
		}
	}
	data, _ := json.MarshalIndent(payload, "", "  ")
...
func main() {
	...
	flag.Parse()
	if *smoke || *smokeFile != "" {
		os.Exit(runSmoke(*smokeFile))
	}
	if *engineSmokeFile != "" {
		os.Exit(runEngineSmoke(*engineSmokeFile))
	}
